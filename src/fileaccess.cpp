...
void FileAccess::addPath( const QString& txt )
{
   ...
   {
      QString currentPath = d()->m_url.path();
      if (currentPath.endsWith(QStringLiteral("/"))) currentPath += QStringLiteral("/");
      currentPath += txt;
      d()->m_url.setPath(currentPath);
...
{
   QUrl dirURL = QUrl::fromLocalFile( dirName );
   if ( dirName.isEmpty() )
      return false;
...
bool FileAccessJobHandler::rename( const QString& dest )
{
   ...
   QUrl kurl = QUrl::fromLocalFile( dest );
   if ( kurl.isRelative() )
      kurl = QUrl::fromLocalFile( QDir().absoluteFilePath(dest) ); // assuming that invalid means relative
...
{
   QUrl parentUrl = QUrl::fromLocalFile( m_pFileAccess->absoluteFilePath() );
   ...
   for ( i=l.begin(); i!=l.end(); ++i )
   {
      ...
      if ( fa.fileName() != "." && fa.fileName() != ".." )
      {
         ...
         QString path = fa.d()->m_url.path();
         if (!path.endsWith(QStringLiteral("/"))) path += QStringLiteral("/");
         path += fa.fileName();
         fa.d()->m_url.setPath(path);
